...
# ```julia
# using Pkg;pkg"add Oceananigans, JLD2, Plots"
...
grid = RegularRectilinearGrid(size = (256, 256), extent=(2π, 2π), 
                              topology=(Periodic, Bounded, Flat))

model = NonhydrostaticModel(timestepper = :RungeKutta3,
                              advection = WENO5(),
                                   grid = grid,
                               coriolis = BetaPlane(f₀=1, β=1),
                               buoyancy = nothing,
                                tracers = nothing,
                                closure = IsotropicDiffusivity(ν=5e-7)
                           )
...
simulation = Simulation(model, Δt=0.02, stop_time=250, iteration_interval=100, progress=progress)
...
anim = @animate for (i, iteration) in enumerate(iterations)
    ...
    s_lim = 0.2
    s_levels = range(0, stop=s_lim, length=40)
...
mp4(anim, "two_dimensional_turbulence.mp4", fps = 12) # hide
